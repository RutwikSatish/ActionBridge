...
Celonis Academy: Academic Process Mining Fundamentals (Apr 2026)

NOTE: All data is synthetically generated for demonstration purposes.
SAP transactions are DRAFTS only — no live SAP connection exists.
"""
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("""
    <div style='font-size:10px; color:#6B7280; text-align:center; line-height:1.8;'>
        ...
        <br><span style='color:#4B5563; font-size:9px;'>
        PR Newswire, June 12, 2025</span>
        <br><br>
        ...
        <span style='color:#F59E0B;'>Celonis Academy: PM Fundamentals</span><br>
        <span style='color:#4B5563; font-size:9px;'>
        Synthetic data · SAP drafts only · No live ERP connection</span>
    </div>
    """, unsafe_allow_html=True)
...
with k6: st.metric("Time Saved/Incident",
                   ...
                   f"vs {stats['manual_process_mins']}m manual (illustrative)",
                   delta_color="normal")
...
# ─────────────────────────────────────────────────────────────────────────────
# TAB 4 — SAP Drafts  [renamed from "SAP Actions" — these are drafts, not live pushes]
# ─────────────────────────────────────────────────────────────────────────────
with tab4:
    st.markdown(f'<p class="ab-label">SAP Transaction Drafts — {sel_shipment} ({len(transactions)} drafts ready)</p>',
                unsafe_allow_html=True)

    st.markdown("""
    <div class="ab-card-amber">
        <div class="ab-label">What These Are</div>
        <div style='font-size:12px; color:#F9FAFB; line-height:1.7;'>
            These are <b>pre-filled SAP transaction drafts</b> generated from the decision engine output.
            A logistics operator reviews each draft and executes it in SAP — replacing manual
            data entry across multiple t-codes. No live SAP connection exists in this prototype.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with t1: st.metric("Manual Process Time", f"{ts['manual_minutes']}m",
                        f"{len(MANUAL_PROCESS_STEPS)} steps (illustrative)", delta_color="off")
    with t2: st.metric("With ActionBridge", f"{ts['auto_minutes']:.0f}m",
                        f"{ts['transactions']} drafts generated", delta_color="off")
    with t3: st.metric("Time Saved", f"{ts['saved_minutes']:.0f}m",
                        f"{ts['reduction_pct']:.0f}% reduction (illustrative)", delta_color="normal")
    with t4: st.metric("Drafts Ready", str(ts["transactions"]),
                        "Awaiting operator review", delta_color="off")

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<p class="ab-label">Generated SAP Transaction Drafts</p>', unsafe_allow_html=True)
    ...
    for txn in transactions:
        ...
        <div class="sap-block">
            ...
            <div style='display:flex; gap:20px; margin-top:10px; font-size:10px;'>
                <span style='color:#10B981;'>⚡ With ActionBridge: {txn.get("estimated_time","< 30s")}</span>
                <span style='color:#EF4444;'>⏱ Manual entry: {txn.get("vs_manual","20+ min")}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.download_button("⬇️  Download SAP Transaction Drafts (CSV)",
                       ...
                       f"actionbridge_{sel_shipment}_drafts.csv",
...
with tab5:
    ...
    <div class="ab-card-amber">
        ...
        <div style='font-size:14px; color:#F9FAFB; line-height:1.7;'>
            ...
            <br><small style='color:#6B7280;'>— PR Newswire, project44 Decision Intelligence launch, June 12, 2025</small>
            <br><br>
            Illustrative benchmark scenario: a project44 alert fires → analyst spends
            <b style='color:#EF4444;'>230 minutes across 10 manual steps</b>
            to get from alert to SAP action. 4–5 people typically involved. Decisions made on
    ...
    st.markdown('<p class="ab-label">Manual Process Steps (Illustrative Benchmark — Before ActionBridge)</p>',
                unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div class="ab-card-success">
        <div class="ab-label">Annual Business Case Summary (Illustrative)</div>
        ...
        <div style='font-size:11px; color:#6B7280; margin-top:12px;'>
            Based on {stats["incidents_per_month"]} incidents/month · $85/hr blended rate (assumption) ·
            65% penalty recovery rate (assumption) · Synthetic simulation data
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with tab6:
    ...
    <div style='background:#111827; border:1px solid #1F2937; border-radius:10px;
        ...
        <div style='font-size:11px; color:#6B7280; line-height:1.8;'>
            ...
            "AI that doesn't lead to action is just another dashboard."
            <br>
            <small style='color:#4B5563;'>PR Newswire, June 12, 2025</small>
            <br><br>
            ActionBridge closes the gap between visibility alert and SAP transaction draft —
            turning a multi-step manual workflow into structured, operator-ready drafts.
            <br><br>
            <b style='color:#F9FAFB;'>Rutwik Satish</b> ·
            MS Engineering Management, Northeastern University (May 2026) ·
            SAP S/4HANA · McKinsey Forward ·
            <span style='color:#F59E0B;'>
            Celonis Academy: Academic Process Mining Fundamentals (Apr 2026)</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with tab7:
    ...
    <div class="ab-card-amber">
        ...
        <div style='font-size:12px; color:#F9FAFB; line-height:1.8;'>
            ActionBridge works with <b>4 interconnected synthetic datasets</b> — Shipments (project44-style events),
            Orders (SAP sales orders), Inventory (SAP MM stock levels), and Carriers (reliability profiles).
            All data is generated with a random seed for demonstration purposes.
            Download any dataset to explore it in Excel.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if "Shipments" in dataset_choice:
        ...
        col_info  = {
            ...
            "disruption_type":   "Root cause category of delay",
        ...
        col_info = {
            ...
            "customer_tier":        "A = strategic, B = standard, C = basic SLA",
        ...
        col_info = {
            ...
            "safety_stock":  "SAP safety stock level",
        ...
        col_info = {
            ...
            "avg_delay_hrs":      "Average delay when late (hours)",
            "capacity":           "Capacity tier (high/medium/low)",
            "cost_index":         "Relative cost multiplier vs base rate",
    ...
    <div style='font-size:10px; color:#6B7280; margin-top:8px; line-height:1.6;'>
        💡 The Shipments dataset is structured to map to a process mining event log.
        A full import would require expanding each shipment into multiple timestamped
        activity rows (e.g. Shipment Created → Delay Detected → Alert Fired → Action Taken)
        with Case ID = shipment_id. The schema here supports that transformation.
    </div>
    """, unsafe_allow_html=True)
...
with tab8:
    ...
    <div class="ab-card-alert">
        <div class="ab-label">🔴 Executive Statement</div>
    ...
    st.markdown("""
    <div class="ab-card">
        <div class="ab-label">The Situation (Illustrative Benchmark Scenario)</div>
        <div style='font-size:13px; color:#F9FAFB; line-height:1.8;'>
            ...
            <br><br>
            But turning that visibility into <b>action</b> still requires — in a typical
    ...
    <div style='background:#EF444415; border:1px solid #EF444430; border-radius:7px;
        ...
        4–5 people typically involved · Illustrative benchmark scenario
    </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    steps = [
        ...
        ("3", "🧠", "Response Options Ranked",
         "The decision engine scores 5 response options (expedite, alt carrier, alt warehouse, reroute, notify) by net benefit = (penalty avoided × success rate) / added cost. Feasibility validated against inventory and carrier data.",
         "#3B82F6"),
        ("4", "✅", "Decision Validated",
         "Before outputting a recommendation, ActionBridge runs a validation layer: checks data completeness, confirms feasibility, and computes a confidence score.",
         "#8B5CF6"),
        ("5", "⚙️", "SAP Transaction Drafts Generated",
         "Based on the chosen response, ActionBridge generates pre-filled SAP transaction drafts: VA02 (delivery date change), ME22N (expedite PO), LT01 (transfer order), VT02N (carrier change), and customer notification templates. A human operator reviews and executes.",
         "#10B981"),
        ("6", "💰", "ROI Quantified",
         "Every decision is tied to a dollar outcome: penalty avoided, freight premium cost, net benefit. 12-month projection calculated from simulation parameters.",
         "#10B981"),
    ]
    ...
    skills = [
        ("SAP S/4HANA Domain Knowledge",
         "Transaction codes and data structures are real — VA02, ME22N, LT01, VT02N. The decision engine maps each disruption type to the correct SAP action. Custom reason codes (Z-prefix) are examples of client-specific configurations.",
         "#F59E0B"),
        ("Celonis Academy: Process Mining Fundamentals",
         "Credential: Academic Process Mining Fundamentals, Celonis Academy, April 2026 (Credly verified). The shipment event log is structured to support process mining analysis — each shipment can be expanded into a multi-activity event log for Celonis import.",
         "#F59E0B"),
        ...
        ("Data Validation Framework",
         "4-dataset quality checks, FK integrity validation, confidence scoring, and decision validation against rule-based baseline.",
         "#3B82F6"),
        ("McKinsey Problem-Solving Structure",
         "Situation (alert detected) → Complication (multi-step manual process) → Resolution (structured decision with quantified ROI). Every screen answers a business question, not a technical one.",
         "#8B5CF6"),
        ("AI Integration (Groq / Llama 3.3 70B)",
         "The AI Analyst Brief uses structured decision output as context. The prompt is designed like a consulting brief — specific numbers, specific action, specific process recommendation.",
         "#8B5CF6"),
    ]
    ...
    <div style='background:#111827; border:1px solid #1F2937; border-radius:10px;
        ...
        <div style='font-size:11px; color:#6B7280; line-height:1.9;'>
            <b style='color:#F59E0B;'>ActionBridge</b> demonstrates the thesis
            Jett McCandless (CEO, project44) described at the Decision Intelligence launch
            (PR Newswire, June 12, 2025):<br>
            ...
            <br>
            <span style='color:#4B5563; font-size:9px;'>
            All data is synthetically generated · SAP transactions are drafts only · No live ERP connection
            </span>
        </div>
    </div>
    """, unsafe_allow_html=True)
